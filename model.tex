...
on links from faulty processes may occur will be called an {\tmem{$( n,m,d )$-system}. For the results established in this paper, we always assume $m>0$, $d>0$, and $n > 
...
and $P^{*}$ to denote all the strings including the empty one.
 ...
 A {\tmem{$k$-round scenario}} (for $(n,m,d)$-system $P$) describes an
